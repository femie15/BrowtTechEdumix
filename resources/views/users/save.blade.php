<x-ux::layouts.modal :title="!$user->exists ? __('New Student Biodata') : __('Update Student Biodata')" submit="save">
    ...
    if($getClass){
     ...
     if($getParent){  
     foreach ($getParent as $key => $name){    
         $parnt[$name]=$key;
     }      
    }
     if($getSection){
     foreach ($getSection as $key => $name){    
         $sect[$name->id]=$name->name.'('.$name->nick_name.')';
     }
    }
          if($getSect){
     foreach ($getSect as $key => $name){    
         $sect[$name->id]=$name->name.'('.$name->nick_name.')';
     }
    }
...
$path_to_file ='assets/images/users/'.$user->id.'.jpg'; 
    ...
    <x-slot name="body">
     ...
     @if(!$user->exists)
        <x-ux::input :label="__('Email')" type="email" model="email" required/>
     @else
        <x-ux::input :label="__('Email')" type="email" model="email" disabled/>
     @endif
     {{-- {{ $getSect }} --}} 
        ...
        <x-ux::select :label="__('Class')" :options="$pt" placeholder="Select Class" model="class_id" wire:change="getSection($event.target.value)" class="form-control" required/>        
        <x-ux::select :label="__('Section')" :options="$sect" placeholder="Select Section" model="section_id" class="form-control" required/>
        <x-ux::select :label="__('Parent')" :options="$parnt" placeholder="Select Parent" model="parent_id" class="form-control"/>
