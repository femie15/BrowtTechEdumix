...
class Elibrarys extends Component
{
    ...
    {
        return Route::get('elibrarys', static::class)
            ->name('elibrarys')
            ->middleware('auth');
    }
    ...
    {
        $query = Topic::query()->where('school_id', Auth::user()->school_id)->where('class_id', Auth::user()->class_id);
